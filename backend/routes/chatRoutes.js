  ...
  try {
    ...
    const stepContext = {
      ...
      content: [
        {
          type: 'input_text',
          text: `User is currently on step "${currentStep}": ${getStepDescription(currentStep)}`,
        },
      ],
    };
    ...
    stepConversations[currentStep] = [...stepHistory, userMessage, assistantMessage];

    if (stepConversations[currentStep].length > 20) {
      const welcomeMessage = stepConversations[currentStep][1];
      stepConversations[currentStep] = [systemMessage, welcomeMessage, ...stepConversations[currentStep].slice(-18)];
    }
